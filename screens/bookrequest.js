...
export default class BookRequest extends Component{
    ...
    Addrequest=async(bookname,reason)=>{
        ...
        var books = await BookSearch.searchbook(bookname,"AIzaSyBmjS79UZcya69uRF6auhglZUGv661KyCk")
    ...
    renderItem=({item,i})=>{
        var object = {title:item.volumeInfo.title,selfLink:item.selfLink,buyLink:item.saleInfo.buyLink,imageLink:item.volumeInfo.imageLinks}
        return(
            <TouchableHighlight style={{alignItems:"center",backgroundColor:"blue",width:"90%"}} activeOpacity={0.5} underlayColor="black" onPress={()=>{
        ...
        return(
            ...
            <View style={{flex:1}}>
               <Myheader title="Requestbooks" navigation ={this.props.navigation}/>
                   ...
                   onChangeText={(text)=>{
                    this.getrequestbookname(text)
                   }}
                   onClear = {text=>this.getrequestbookname("")}
                   value={this.state.bookname}
                   />{this.state.showflatlist?(<FlatList data={this.state.datasource} renderItem={this.renderItem} 
